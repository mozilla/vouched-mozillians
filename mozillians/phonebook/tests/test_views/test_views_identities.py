...
class IdentitiesViewsTests(TestCase):
    ...
    def test_change_identity_of_another_user(self):
        ...
        with self.login(user) as client:
            url = reverse(
                "phonebook:change_primary_contact_identity",
    ...
    def test_change_valid_identity(self, mocked_message):
        ...
        ok_(
            IdpProfile.objects.filter(
                pk=idp_primary.pk, primary_contact_identity=True
            ).exists()
        )
        ...
        ok_(
            IdpProfile.objects.filter(
                pk=idp_primary.pk, primary_contact_identity=False
            ).exists()
        )
        ok_(
            IdpProfile.objects.filter(pk=idp.pk, primary_contact_identity=True).exists()
        )
