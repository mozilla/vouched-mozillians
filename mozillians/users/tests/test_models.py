...
from mozillians.groups.tests import (
    GroupAliasFactory,
    GroupFactory,
    SkillAliasFactory,
    SkillFactory,
...
class SignaledFunctionsTests(TestCase):
    ...
    def test_vouch_multiple_mozilla_alternate_emails(self):
        ...
        vouch_query = Vouch.objects.filter(
            vouchee=user.userprofile,
            autovouch=True,
...
class UserProfileTests(TestCase):
    ...
    def test_accounts_access(self):
        ...
        user.userprofile.externalaccount_set.create(
            type=ExternalAccount.TYPE_SUMO, identifier="test"
        )
    ...
    def test_accounts_public_mozillians(self):
        ...
        profile.set_instance_privacy_level(PUBLIC)
        profile.externalaccount_set.create(
            type=ExternalAccount.TYPE_SUMO, identifier="test", privacy=MOZILLIANS
        )
    ...
    def test_annotated_tags_not_public(self):
        ...
        profile = UserFactory.create(
            userprofile={"privacy_groups": MOZILLIANS}
        ).userprofile
    ...
    def test_email_now_vouched_can_vouch(self, get_template_mock, send_mail_mock):
        ...
        user.userprofile._email_now_vouched(None)
        eq_(
            get_template_mock().render.call_args_list[0][0][0]["can_vouch_threshold"],
            False,
        )
        Vouch.objects.create(voucher=None, vouchee=user.userprofile, date=now())
        user.userprofile._email_now_vouched(None)
        eq_(
            get_template_mock().render.call_args_list[1][0][0]["can_vouch_threshold"],
            True,
        )
    ...
    def test_is_public(self):
        for field in UserProfile.privacy_fields():
            user = UserFactory.create(userprofile={"privacy_{0}".format(field): PUBLIC})
            ok_(
    ...
    def test_is_public_indexable(self):
        for field in PUBLIC_INDEXABLE_FIELDS:
            user = UserFactory.create(
                userprofile={"privacy_{0}".format(field): PUBLIC, "ircname": "bar"}
            )
            ok_(
                user.userprofile.is_public_indexable,
    ...
    def test_set_membership_system_group(self):
        ...
        group_1 = GroupFactory.create(
            visible=False, members_can_leave=False, accepting_new_members="no"
        )
        user = UserFactory.create()
    ...
    def test_get_photo_thumbnail_with_photo(
        self, get_thumbnail_mock, mock_image, mock_storage
    ):
        mock_image_obj = Mock()
...
class VouchTests(TestCase):
    ...
    def test_vouchee_privacy(self):
        ...
        user_profile.set_instance_privacy_level(PUBLIC)
        eq_(
            set(user_profile.vouches_made.all()),
        ...
        eq_(
            set(user_profile.vouches_made.all()),
            set(Vouch.objects.filter(voucher=user_profile)),
        )
...
class ExternalAccountTests(TestCase):
    ...
    def test_get_url_unicode(self):
        profile = UserFactory.create().userprofile
        account = profile.externalaccount_set.create(
            type=ExternalAccount.TYPE_MDN, identifier="sammyウ"
        )
...
class EmailAttributeTests(TestCase):
    ...
    def test_not_existing_idp_privacy_allowed(self):
        userprofile_args = {"privacy_email": PUBLIC}
        profile = UserFactory.create(
            email="foo@foo.com", userprofile=userprofile_args
        ).userprofile
        profile.set_instance_privacy_level(MOZILLIANS)
        eq_(profile.email, "foo@foo.com")
...
class CISHelperMethodsTests(unittest.TestCase):
    ...
    def test_cis_groups_highest(self):
        user = UserFactory.create()
        group1 = GroupFactory.create(name="nda", is_access_group=True)
        group2 = GroupFactory.create(name="cis_whitelist", is_access_group=True)
        group3 = GroupFactory.create(
            name="open innovation + reps council", is_access_group=True
        )
        ...
        eq_(
            set(user.userprofile.get_cis_groups(idp)),
            set(
                [
                    "mozilliansorg_nda",
    ...
    def test_cis_tags(self):
        user = UserFactory.create()
        group1 = GroupFactory.create(name="foo", is_access_group=False)
        group2 = GroupFactory.create(name="bar", is_access_group=False)
        group3 = GroupFactory.create(name="baz", is_access_group=False)
        group1.add_member(user.userprofile)
        group2.add_member(user.userprofile)
        group3.add_member(user.userprofile, status="PENDING")
        IdpProfile.objects.create(
        ...
        eq_(set(user.userprofile.get_cis_tags()), set(["foo", "bar"]))
